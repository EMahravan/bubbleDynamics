...
def seperate( timeName ):
  if (float(timeName)>1e-6 and float(timeName)<1.3e-6):
    ...
    for i in freeSurface:
        writeRaw.write(str(pointXSorted[i,0])+"  "+str(pointXSorted[i,1])+"  "+str(pointXSorted[i,2])+"   "+str(pointXSorted[i,3])+"\n")
...
##############################################################
    ...
    writeRaw = open(writeDestination, "w")
    for i in freeSurface:
        writeRaw.write(str(pointYSorted[i,0])+"  "+str(pointYSorted[i,1])+"  "+str(pointYSorted[i,2])+"   "+str(pointYSorted[i,3])+"\n")
